...
자동 설정되어 있지 않으면 다음과 같은 코드를 환경 설정 파일에 추가
...
위의 설정이 되면, pyenv 환경 설정이 적용이 되면 다음과 같이 프롬프트가 변경됨
...
익스텐션을 사용하려면 시스템(맥이나 윈도우 환경 등)에 Node가 설치 되어 있어야 한다. 노드 설치는 [여기](./2020-02-17-Node설치.md)에 작성해 두었다.
익스텐션 사용법은 [여기](https://jupyterlab.readthedocs.io/en/stable/user/extensions.html)에서 살펴보면 된다. 
