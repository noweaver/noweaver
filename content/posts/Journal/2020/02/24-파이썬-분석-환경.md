...
Anaconda 경우는 점점 비대해지고 있고 설치나 실행 성능이 느려지고 있음을 깨닫는다. 그거의 라이트(light) 버전인 Miniconda를 설치한다.
...
자동 설정되어 있지 않으면 다음과 같은 코드를 환경 설정 파일에 추가한다. 일반적으로는 자동으로 환경 변수에 코드 삽입이 됨. 확인해 볼 것.
